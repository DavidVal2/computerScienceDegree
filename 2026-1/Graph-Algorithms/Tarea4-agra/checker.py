...
def check(solution_file, in_file, out_file):
    ...
    if my_output == expected:
        ...
        # 🔥 NUEVO: comparación línea por línea
        print("------ Diferencias por línea ------")
        expected_lines = expected.splitlines()
        my_lines = my_output.splitlines()

        max_len = max(len(expected_lines), len(my_lines))

        for i in range(max_len):
            exp_line = expected_lines[i] if i < len(expected_lines) else "<VACÍO>"
            my_line = my_lines[i] if i < len(my_lines) else "<VACÍO>"

            if exp_line != my_line:
                print(f"Línea {i+1}:")
                print(f"  Esperado: {repr(exp_line)}")
                print(f"  Obtenido: {repr(my_line)}")
